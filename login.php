<?php
  ...
  if (isset($_SESSION) && !empty($_SESSION["account_id"])) {
  ...
  if (isset($_COOKIE["remembered"]) && !empty($_COOKIE["remembered"])) {
  ...
  <form class="form-signin" method="post" action="login.php">
    ...
    <div>
      ...
      <div class="checkbox text-white mb-3">
        <label>
          <input type="checkbox" id="remember_me" name="remembered" class="mr-2" value="remember-me">Ricordami su questo dispositivo
        </label>
      </div>
      <button id="login" class="btn btn-lg btn-<?= ($login_failed && empty($account)) ? "danger" : "success" ?> btn-block" type="submit"><?= ($login_failed && empty($account)) ? "Chi?" : "Accedi" ?></button>
    </div>
  </form>
<?php
  }
